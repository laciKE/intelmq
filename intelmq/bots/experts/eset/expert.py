...
# -*- coding: utf-8 -*-
"""
Expert bot for ESET Threat Intelligence feeds
This bot  analyzes event's comments based on STIX indicator's description
...
class ESETExpertBot(ExpertBot):
    ...
    def classify(comment):
        ...
        for (pattern, classification_type) in CLASSIFICATION_BY_REGEX.items():
            ...
            if match:
                ...
                if len(groups) > 0:
                    malware = groups[0]
                    malware_name = ESETExpertBot.extract_malware_family(malware)
                return (classification_type, malware_name)
    ...
    def extract_malware_family(malware):
        ...
        match = ESETExpertBot._malware_naming_convention_pattern.match(malware)
